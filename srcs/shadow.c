...
{
	if (((ft_compare_float(len_vec(*dsph), r) > 0 && ft_compare_float(len_vec(*lsph), r) > 0))
		&& (ft_compare_float(scalar_vec(*dsph, *dist->dot_light), 0) > 0
		&& ft_compare_float(scalar_vec(*lsph, *dist->dot_light), 0) < 0))
		if ((ft_compare_float(len_vec(cross_prod_vec(*dsph, *dist->dot_light))
			/ len_vec(*dist->dot_light), r) < 0)
			|| ft_compare_float(len_vec(cross_prod_vec(*dsph, *lsph)) / len_vec(*dist->dot_light), 0) == 0)
				return (1);
	return (0);
}
...
{
	if ((ft_compare_float(len_d_s, r) < 0 && ft_compare_float(len_l_s, r) > 0) || (ft_compare_float(len_d_s, r) > 0 && ft_compare_float(len_l_s, r) < 0))
		return (1);
	return (0);
}
...
int	shadow_sphere(t_minirt *mt, t_dist *dist, t_vector *ray)
{
	...
	{
		sp = ptr->content;
		if (ft_compare_float(dist->closest_obj, 1) != 0 || (ft_compare_float(dist->closest_obj, 1) == 0
			&& (ft_compare_float(sp->center.x, dist->cl_sp->center.x) != 0
			&& ft_compare_float(sp->center.y, dist->cl_sp->center.y) != 0
			&& ft_compare_float(sp->center.z, dist->cl_sp->center.z) != 0) && (ft_compare_float(sp->diam, dist->cl_sp->diam) != 0)))
		{
...
int	shadow_plane(t_minirt *mt, t_dist *dist, t_vector *ray)
{
	...
	{
		pl = ptr->content;
		if (ft_compare_float(dist->closest_obj, 2) != 0 || (ft_compare_float(dist->closest_obj, 2) == 0
			&& (ft_compare_float(pl->coord.x, dist->cl_pl->coord.x) != 0
			&& ft_compare_float(pl->coord.y, dist->cl_pl->coord.y) != 0
			&& ft_compare_float(pl->coord.z, dist->cl_pl->coord.z) != 0)
			&& (ft_compare_float(pl->orientation.x, dist->cl_pl->orientation.x) != 0
			&& ft_compare_float(pl->orientation.y, dist->cl_pl->orientation.y) != 0
			&& ft_compare_float(pl->orientation.z, dist->cl_pl->orientation.z) != 0)))
		{
...
static float	shadow_disc_intersect(t_vector *dot_light, t_vector *ray, t_plane *plane, float r)
{
	...
	if (ft_compare_float(t, 0) != 0)
	{
		mult_vec(dot_light, t);
		p = add_vec(*ray, *dot_light);
		v = sub_vec(p, plane->coord);
		dist = sqrtf(scalar_vec(v, v));
		if ((ft_compare_float(p.x, plane->coord.x) == 0 && ft_compare_float(p.y, plane->coord.y) == 0 && ft_compare_float(p.z, plane->coord.z) == 0) || ft_compare_float(dist, r) <= 0)
			return (1);
	}
	return (0);
}
